...
## バーの長さの基準(renderer/app.js の render())

バーの幅は「今日の総額に対するそのモデルの割合(`d.v / total`)」で描く。右側に出す
「%」表記と同じ基準にするため。当初は「最大値に対する相対幅(`d.v / max`)」で描いて
いたが、上位2モデルの金額が近い時に両方ほぼ満タンに見え、「50% なのにバーが両方フル?」
という混乱を招いた(実際に指摘された)。% とバー長は必ず同じ基準で揃えること。

...
const PRICING = [
  ...
  // Sonnet 5 の導入価格($2/$10, 2026-08-31まで)は、実際の /usage の課金額と突き合わせて
  // 検証した結果、Team シートの extra usage 課金では適用されていなかった(標準価格 $3/$15
  // で計算した方が実測値に 0.7% 差まで一致した — lib/usage.mjs 変更履歴参照)。
  // 直接の pay-as-you-go API キー利用者では導入価格が効く可能性があるが、実測できないため
  // ここでは標準価格に統一する。ズレを見つけたら CLAUDE.md の「単価テーブル」の項目を参照。
...
function listTranscripts() {
  ...
  // readdirSync の { recursive: true } は Node v20.1+ 限定(README は Node 18+ を要求)。
  // 幅広い互換性のため、withFileTypes だけで自前再帰する。
  /** @type {string[]} */
  const out = [];
  const stack = [base];
  while (stack.length) {
...
export function collect() {
  ...
  // 1つの応答(1回の課金)が thinking/text/tool_use など複数行に分かれて記録され、
  // 各行が同一の usage を丸ごと繰り返し持つ(実測で確認済み: 同じ message.id の行は
  // usage が完全に同一)。message.id 単位で1回だけ数えないと、ほぼ2〜3倍に過大集計される。
  ...
  for (const file of listTranscripts()) {
    ...
    for (const line of text.split("\n")) {
      ...
      // currentModel の判定(直近の応答がどのモデルか)は重複行でも成立するので
      // dedupe より前に更新してよいが、コストの重複加算だけは防ぐ。
      if (ts >= latestTs) { latestTs = ts; currentModel = priceOf(msg.model).label; }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // (以前は「セッション合計」も表示していたが、意味が「最後にどのファイルを触ったか」に
  //  依存して不安定だったため削除した。詳細は CLAUDE.md 参照。)
  console.log("\n--- JSON ---");

...
- **透明ウィンドウの W/H はカード本体より一回り大きくする(main.js の `W`/`H`)** — カードには
  `box-shadow: 0 8px 32px`(blur 32px + y8px)が付いている。透明ウィンドウの端は影を切り取るので、
  カード周囲に blur 分の透明余白(index.html の `.widget { margin }`)が無いと影がハードに断ち切られ、
  **カードの外周に暗い矩形の縁が出る**(実際に発生: margin 6px / W300×H250 の頃に影が潰れて見えた)。
  現在は `margin:24px` + `W336×H300` で 32px blur が収まるようにしている。影のサイズを変えるなら
  ウィンドウサイズと margin も必ず一緒に見直すこと。明るい壁紙の上ほどこの断ち切りが目立つ。
...
## 表示モードのトグル(使用上限 / 使用量分析)

ヘッダー右のセグメントボタン2個で `.bars` の中身を切り替える(renderer/app.js の `mode` / `setMode()`)。
アクティブ側に `.on`(青 = `--fable`)を付ける。既定は `limit`(使用上限)。

- **使用上限(`limit`)** — `limitBars()`。今日の総額 / `dailyMax` を1本のバーで描く。しきい値は
  〜75% 緑 / 〜100% 橙 / 超過 赤。幅は 100% で頭打ち。
- **使用量分析(`analysis`)** — `analysisBars()`。従来のモデル別割合バー(総額に対する割合、% と幅は同基準)。

なぜこの2モードにしたか(重要): ユーザーは当初、モデル別割合バーを見て「`/usage` のように
**上限のうち何%使ったか**を出しているもの」と誤解した。`/usage` の「X% used / 週リセット」は
**サーバー側のサブスク レート制限**で、Claude Code が認証付きの内部 API から取得している。
このデータは `~/.claude` のトランスクリプトには存在せず、ローカル専用・外部通信は為替のみ、という
本アプリの制約では再現できない(技術的に不可能ではないが、非公開エンドポイント + 認証情報の読取が
必要で、Claude Code のバージョン更新で壊れる。やらない)。そこで「上限」は **ローカルの1日予算
(`dailyMax`、猫のしきい値と共有)** に対する割合と定義した。`/usage` とは軸が違うことを README にも明記済み。

- ヘッダーの旧 `· today` 表記はこのトグル追加時に削除した(冗長 + ボタンの場所確保のため)。
- トグルは IPC を待たず即再描画する(`lastData` を保持し、クリック時に `render(lastData)`)。
